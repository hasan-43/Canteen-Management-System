...
// Fetch products from olympia table in database
$catalog = [];
$dbError = '';
$sql = "SELECT olympia_id, product_code, name, price, category, image, stock, is_available 
        FROM olympia 
        WHERE is_available = 1 
        ORDER BY category, name";
$result = $conn->query($sql);
if (!$result) {
    $dbError = "Database Error: " . $conn->error . "<br><strong>Did you import olympia_table.sql?</strong>";
    ...
    $dbError = "No products found in olympia table. Please import olympia_table.sql";
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && isset($_POST['add_to_cart'])) {
    ...
    if ($item && isset($_SESSION['inventory'][$itemId])) {
        ...
        if ($qty <= $available) {
            ...
            $stmt = $conn->prepare("SELECT item_id, quantity FROM cart_items WHERE cart_id = ? AND product_code = ? AND kitchen = 'olympia'");
            ...
            if ($result->num_rows > 0) {
                ...
                // Insert new item
                $stmt = $conn->prepare("INSERT INTO cart_items (cart_id, product_code, product_name, kitchen, price, quantity) VALUES (?, ?, ?, 'olympia', ?, ?)");
...
<body class="bg-gray-100 text-gray-900">
    ...
    <div class="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between gap-6">
      ...
      <nav class="nav-buttons text-sm">
        ...
        <div class="nav-dropdown">
          <button class="nav-link hover:bg-red-50">Shop ▾</button>
      ...
      <div class="flex items-center gap-4">
        <div class="relative" id="profileRoot">
          <button id="profileBtn" class="flex items-center gap-2 p-1 rounded hover:bg-gray-100">
            <?php if (!empty($profilePic)): ?>
              <img src="<?= htmlspecialchars($profilePic) ?>" alt="Profile" class="w-10 h-10 rounded-full object-cover border border-gray-300" />
            <?php else: ?>
              <div class="initials-circle"><?= htmlspecialchars($initials) ?></div>
            <?php endif; ?>
            <span class="hidden sm:inline text-sm font-semibold text-gray-800"><?= htmlspecialchars($displayName) ?></span>
          </button>
          <div id="profileMenu" class="profile-menu hidden absolute right-0 mt-2 w-44 bg-white rounded shadow-lg border border-gray-200 text-sm">
            <a href="./profile.php" class="block px-4 py-2 hover:bg-gray-100">Profile</a>
            <a href="../logout.php" class="block px-4 py-2 text-red-600 hover:bg-gray-100">Log out</a>
          </div>
        </div>
      </div>
    </div>
  ...
  <section id="hero" class="hero" style="background-image: url('../resources/sign_up.jpg');">
    ...
    <div class="relative z-10 max-w-4xl mx-auto px-4 py-24 text-center text-white">
      <h1 class="text-4xl sm:text-5xl font-extrabold mb-4">Olympia Kitchen</h1>
      ...
      <?php foreach ($catalog as $item): ?>
        ...
        <div class="bg-white rounded-xl shadow hover:shadow-md transition overflow-hidden border border-gray-100">
          <div class="h-48 bg-gray-200 overflow-hidden">
            <img src="<?= '../resources/Olympia/' . rawurlencode($item['image']) ?>" alt="<?= htmlspecialchars($item['name']) ?>" class="w-full h-full object-cover" />
          </div>
