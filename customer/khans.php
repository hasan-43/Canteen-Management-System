...
// Fetch products from khans table in database with ratings
$catalog = [];
$dbError = '';
$sql = "SELECT k.khans_id, k.product_code, k.name, k.price, k.category, k.image, k.stock, k.is_available,
        COALESCE(AVG(pr.rating), 0) as avg_rating,
        COUNT(pr.review_id) as review_count
        FROM khans k
        LEFT JOIN product_reviews pr ON k.product_code = pr.product_code AND pr.kitchen = 'khans'
        WHERE k.is_available = 1
        GROUP BY k.khans_id, k.product_code, k.name, k.price, k.category, k.image, k.stock, k.is_available
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && isset($_POST['add_to_cart'])) {
    ...
    if ($item) {
      ...
      $stmt = $conn->prepare("SELECT stock FROM khans WHERE product_code = ? FOR UPDATE");
      ...
      $stmt = $conn->prepare("UPDATE khans SET stock = stock - ? WHERE product_code = ? AND stock >= ?");
            ...
            <div class="bg-white rounded-[2rem] border border-gray-100/90 shadow-sm hover:shadow-2xl hover:-translate-y-1.5 transition duration-500 p-4 flex flex-col justify-between h-full">
              <div class="relative p-2.5 bg-gray-50/60 rounded-[1.5rem] border border-gray-100/80 overflow-hidden flex-shrink-0 mb-4">
                <div class="aspect-[4/3] w-full rounded-[1.25rem] overflow-hidden bg-white relative">
                  <img src="<?= getHDProductImage($item['name'], 'khans', $item['image']) ?>" alt="<?= htmlspecialchars($item['name']) ?>" class="w-full h-full object-cover transition-transform duration-500 hover:scale-110" />
                </div>
  ...
  <script>
    ...
    document.querySelectorAll('.view-reviews-btn').forEach(btn => {
      btn.addEventListener('click', function() {
        ...
        // Fetch reviews
        fetch(`../api/reviews.php?product_code=${encodeURIComponent(productCode)}&kitchen=khans`)
          .then(res => res.json())
          .then(data => {
            if (data.success) {
