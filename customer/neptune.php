...
// Fetch products from neptune table in database
$catalog = [];
$dbError = '';
$sql = "SELECT neptune_id, product_code, name, price, category, image, stock, is_available 
        FROM neptune 
      ...
      <?php foreach ($catalog as $item): ?>
        ...
        <div class="bg-white rounded-xl shadow hover:shadow-md transition overflow-hidden border border-gray-100">
          <div class="h-48 bg-gray-200 overflow-hidden">
            <img src="<?= '../resources/Neptune/' . rawurlencode($item['image']) ?>" alt="<?= htmlspecialchars($item['name']) ?>" class="w-full h-full object-cover" />
          </div>
