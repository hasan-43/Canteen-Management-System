...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $image = handleImageUpload($shopTable);
        ...
        if ($name && $price > 0) {
            $stmt = $conn->prepare("INSERT INTO `{$shopTable}` (product_code, name, price, category, image, stock, is_available) VALUES (?, ?, ?, ?, ?, ?, 1)");
            if ($stmt) {
        ...
        if ($item_id > 0 && $name && $price > 0) {
            $stmt = $conn->prepare("UPDATE `{$shopTable}` SET name=?, price=?, category=?, image=?, stock=? WHERE `{$pkName}`=?");
            if ($stmt) {
                $stmt->bind_param("sdssii", $name, $price, $category, $image, $stock, $item_id);
                if ($stmt->execute()) {
        ...
        if ($item_id > 0) {
            $stmt = $conn->prepare("DELETE FROM `{$shopTable}` WHERE `{$pkName}`=?");
            if ($stmt) {
        ...
        if ($item_id > 0) {
            $stmt = $conn->prepare("UPDATE `{$shopTable}` SET is_available=? WHERE `{$pkName}`=?");
            if ($stmt) {
...
// Fetch all products from custom shop table
$products = [];
$sql = "SELECT `{$pkName}` AS item_id, product_code, name, price, category, image, stock, is_available FROM `{$shopTable}` ORDER BY category, name";
...
<HEAD>
    ...
    <title>Product Management - <?= htmlspecialchars($shopDisplayName) ?></title>
    ...
    <style>
        ...
        table.product-table th:nth-child(1), table.product-table td:nth-child(1) { width: 20%; }
        table.product-table th:nth-child(2), table.product-table td:nth-child(2) { width: 18%; }
        table.product-table th:nth-child(3), table.product-table td:nth-child(3) { width: 18%; }
        table.product-table th:nth-child(4), table.product-table td:nth-child(4) { width: 14%; }
        table.product-table th:nth-child(5), table.product-table td:nth-child(5) { width: 12%; }
        table.product-table th:nth-child(6), table.product-table td:nth-child(6) { width: 18%; }
...
<body class="bg-gray-50 text-gray-900">
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <form id="editForm" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <input type="hidden" name="item_id" id="editProductId">
                        ...
                        <tbody id="productTableBody">
                            <?php foreach ($products as $p): ?>
                                <tr data-category="<?= htmlspecialchars($p['category']) ?>">
                                    ...
                                    <td>
                                        <?php if (trim($p['image']) !== ''): ?>
                                            <img src="<?= '../../resources/' . ucfirst($shopTable) . '/' . rawurlencode($p['image']) ?>" alt="<?= htmlspecialchars($p['name']) ?>" style="width:80px;height:60px;object-fit:cover;border-radius:8px;" />
                                        <?php else: ?>
                                    ...
                                    <td><?= (int)$p['stock'] ?></td>
                                    <td>
                                        <button type="button" class="btn btn-secondary" style="padding:0.5rem 0.75rem; font-size:0.875rem;" onclick="editProduct(<?= (int)$p['item_id'] ?>, '<?= htmlspecialchars($p['name'], ENT_QUOTES) ?>', <?= (float)$p['price'] ?>, '<?= htmlspecialchars($p['category'], ENT_QUOTES) ?>', '<?= htmlspecialchars($p['image'], ENT_QUOTES) ?>', <?= (int)$p['stock'] ?>)">Edit</button>
    ...
    <script>
        ...
        (function() {
            const btns = document.querySelectorAll('#categoryFilter button');
