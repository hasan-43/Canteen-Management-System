...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if ($order_id > 0 && !empty($new_status)) {
        $stmt = $conn->prepare("UPDATE orders SET order_status = ? WHERE order_id = ? AND kitchen = ?");
        if ($stmt) {
            $stmt->bind_param("sis", $new_status, $order_id, $shopTable);
            if ($stmt->execute()) {
...
// Fetch all orders for this kitchen with customer details
$orders = [];
...
<HEAD>
    ...
    <title>Order Management - <?= htmlspecialchars($shopDisplayName) ?></title>
...
<body class="bg-gray-50 text-gray-900">
    ...
    <section id="hero" class="hero">
        ...
        <div class="relative z-10 max-w-4xl mx-auto px-4 py-24 text-center text-white">
            ...
            <p class="text-lg text-gray-200"><?= htmlspecialchars($shopDisplayName) ?> Orders</p>
        </div>
    </section>
                        ...
                        <div class="order-header">
                            <div class="flex justify-between items-start flex-wrap gap-3">
                                <div>
                                    ...
                                    <?php $orderPhone = $order['phone_number'] ?? $order['customer_phone'] ?? 'N/A'; ?>
