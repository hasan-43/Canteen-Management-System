...
function handleImageUpload() {
    ...
    $uploadDir = __DIR__ . '/../../resources/Olympia/';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if ($name && $price > 0) {
            $stmt = $conn->prepare("INSERT INTO olympia (product_code, name, price, category, image, stock, is_available) VALUES (?, ?, ?, ?, ?, ?, 1)");
            if ($stmt) {
        ...
        $olympia_id = intval($_POST['olympia_id'] ?? 0);
        $name = trim($_POST['name'] ?? '');
        $price = floatval($_POST['price'] ?? 0);
        $category = trim($_POST['category'] ?? 'mains');
        ...
        if ($olympia_id > 0 && $name && $price > 0) {
            $stmt = $conn->prepare("UPDATE olympia SET name=?, price=?, category=?, image=?, stock=? WHERE olympia_id=?");
            if ($stmt) {
                $stmt->bind_param("sdssii", $name, $price, $category, $image, $stock, $olympia_id);
                if ($stmt->execute()) {
        ...
        if ($olympia_id > 0) {
            $stmt = $conn->prepare("DELETE FROM olympia WHERE olympia_id=?");
            if ($stmt) {
                $stmt->bind_param("i", $olympia_id);
                if ($stmt->execute()) {
...
// Fetch all products from olympia table
$products = [];
$sql = "SELECT olympia_id, product_code, name, price, category, image, stock FROM olympia ORDER BY category, name";
...
<head>
    ...
    <title>Olympia Kitchen - Product Management</title>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <form id="editForm" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <input type="hidden" name="olympia_id" id="editProductId">
                        ...
                        <tbody id="productTableBody">
                            <?php foreach ($products as $p): ?>
                                <tr data-category="<?= htmlspecialchars($p['category']) ?>">
                                    ...
                                    <td>
                                        <?php if (trim($p['image']) !== ''): ?>
                                            <img src="<?= '../../resources/Olympia/' . rawurlencode($p['image']) ?>" alt="<?= htmlspecialchars($p['name']) ?>" style="width:80px;height:60px;object-fit:cover;border-radius:8px;" />
                                        <?php else: ?>
                                    ...
                                    <td><?= (int)$p['stock'] ?></td>
                                    <td>
                                        <button type="button" class="btn btn-secondary" style="padding:0.5rem 0.75rem; font-size:0.875rem;" onclick="editProduct(<?= (int)$p['olympia_id'] ?>, '<?= htmlspecialchars($p['name'], ENT_QUOTES) ?>', <?= (float)$p['price'] ?>, '<?= htmlspecialchars($p['category'], ENT_QUOTES) ?>', '<?= htmlspecialchars($p['image'], ENT_QUOTES) ?>', <?= (int)$p['stock'] ?>)">Edit</button>
                                        <form method="POST" style="display:inline;" onsubmit="return confirm('Delete this product?');">
                                            <input type="hidden" name="action" value="delete">
                                            <input type="hidden" name="olympia_id" value="<?= (int)$p['olympia_id'] ?>">
