...
function handleImageUpload() {
    ...
    $uploadDir = __DIR__ . '/../../resources/Khans/';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if ($name && $price > 0) {
            $stmt = $conn->prepare("INSERT INTO khans (product_code, name, price, category, image, stock, is_available) VALUES (?, ?, ?, ?, ?, ?, 1)");
            if ($stmt) {
        ...
        $khans_id = intval($_POST['khans_id'] ?? 0);
        $name = trim($_POST['name'] ?? '');
        $price = floatval($_POST['price'] ?? 0);
        $category = trim($_POST['category'] ?? 'mains');
        ...
        if ($khans_id > 0 && $name && $price > 0) {
            $stmt = $conn->prepare("UPDATE khans SET name=?, price=?, category=?, image=?, stock=? WHERE khans_id=?");
            if ($stmt) {
                $stmt->bind_param("sdssii", $name, $price, $category, $image, $stock, $khans_id);
                if ($stmt->execute()) {
        ...
        if ($khans_id > 0) {
            $stmt = $conn->prepare("DELETE FROM khans WHERE khans_id=?");
            if ($stmt) {
                $stmt->bind_param("i", $khans_id);
                if ($stmt->execute()) {
        ...
        if ($khans_id > 0) {
            $stmt = $conn->prepare("UPDATE khans SET is_available=? WHERE khans_id=?");
            if ($stmt) {
                $stmt->bind_param("ii", $is_available, $khans_id);
                if ($stmt->execute()) {
...
// Fetch all products from khans table
$products = [];
$sql = "SELECT khans_id, product_code, name, price, category, image, stock, is_available FROM khans ORDER BY category, name";
...
<head>
    ...
    <title>Khans Kitchen - Product Management</title>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <form id="editForm" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <input type="hidden" name="khans_id" id="editProductId">
                        ...
                        <tbody id="productTableBody">
                            <?php foreach ($products as $p): ?>
                                <tr data-category="<?= htmlspecialchars($p['category']) ?>">
                                    ...
                                    <td>
                                        <?php if (trim($p['image']) !== ''): ?>
                                            <img src="<?= '../../resources/Khans/' . rawurlencode($p['image']) ?>" alt="<?= htmlspecialchars($p['name']) ?>" style="width:80px;height:60px;object-fit:cover;border-radius:8px;" />
                                        <?php else: ?>
                                    ...
                                    <td><?= (int)$p['stock'] ?></td>
                                    <td>
                                        <button type="button" class="btn btn-secondary" style="padding:0.5rem 0.75rem; font-size:0.875rem;" onclick="editProduct(<?= (int)$p['khans_id'] ?>, '<?= htmlspecialchars($p['name'], ENT_QUOTES) ?>', <?= (float)$p['price'] ?>, '<?= htmlspecialchars($p['category'], ENT_QUOTES) ?>', '<?= htmlspecialchars($p['image'], ENT_QUOTES) ?>', <?= (int)$p['stock'] ?>)">Edit</button>
                                        <form method="POST" style="display:inline;" onsubmit="return confirm('Delete this product?');">
                                            <input type="hidden" name="action" value="delete">
                                            <input type="hidden" name="khans_id" value="<?= (int)$p['khans_id'] ?>">
