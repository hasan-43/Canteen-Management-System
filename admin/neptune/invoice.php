...
$countSql = "SELECT COUNT(*) as total FROM orders WHERE LOWER(kitchen) = 'neptune' AND order_status = 'delivered'";
...
// Fetch delivered orders for Neptune Kitchen with customer details (paginated)
$orders = [];
$totalRevenue = 0.0;
...
$sql = "SELECT o.*, c.username, c.fullname, c.email, c.phone 
		...
		WHERE LOWER(o.kitchen) = 'neptune' AND o.order_status = 'delivered' 
...
$revSql = "SELECT SUM(total_amount) as total_rev FROM orders WHERE LOWER(kitchen) = 'neptune' AND order_status = 'delivered'";
...
<HEAD>
	...
	<title>Neptune Kitchen - Invoices</title>
...
<body class="bg-gray-50 text-gray-900">
	...
	<script>
		...
		function printInvoice(orderId) {
			...
			const invoiceHTML = `
				<div style="font-family: 'Arial', sans-serif; max-width: 800px; margin: 0; background: linear-gradient(135deg, #fbbf24 0%, #f59e0b 50%, #d97706 100%); padding: 10px; border-radius: 8px; box-shadow: 0 20px 60px rgba(0,0,0,0.3);">
					<!-- Header -->
					<div style="background: white; padding: 18px; border-radius: 10px; margin-bottom: 15px; box-shadow: 0 8px 20px rgba(0,0,0,0.15);">
						<div style="text-align: center; margin-bottom: 15px;">
							<h1 style="color: #f59e0b; font-size: 30px; margin: 0; font-weight: bold; text-transform: uppercase; letter-spacing: 2px; text-shadow: 2px 2px 4px rgba(245,158,11,0.3);">Campus Cravings</h1>
							<p style="color: #d97706; font-size: 16px; margin: 3px 0 0 0; font-weight: 600;">Neptune Kitchen</p>
						</div>
						<div style="border-top: 3px solid #fbbf24; padding-top: 15px;">
							<h2 style="color: #333; font-size: 24px; margin: 0 0 10px 0;">${orderNumber}</h2>
							<div style="display: flex; justify-content: space-between; flex-wrap: wrap;">
								<div>
									<p style="margin: 5px 0; color: #555;"><strong style="color: #f59e0b;">Customer:</strong> ${customerName}</p>
									${phone ? `<p style="margin: 5px 0; color: #555;"><strong style="color: #f59e0b;">Phone:</strong> ${phone}</p>` : ''}
								</div>
					...
					<!-- Items Section -->
					<div style="background: white; padding: 18px; border-radius: 10px; margin-bottom: 15px; box-shadow: 0 8px 20px rgba(0,0,0,0.15);">
						<h3 style="color: #f59e0b; font-size: 18px; margin: 0 0 12px 0; border-bottom: 2px solid #fbbf24; padding-bottom: 8px;">Order Items</h3>
							...
							${Array.from(originalCard.querySelectorAll('.item-row')).map(item => {
								...
								return `
									<div style="display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 1px solid #e5e7eb;">
										...
										<div style="font-weight: bold; color: #f59e0b; font-size: 16px;">${price}</div>
									</div>
								`;
							}).join('')}
						</div>
						...
						<!-- Totals -->
						<div style="margin-top: 20px; padding-top: 15px; border-top: 3px solid #fbbf24;">
							...
							<div style="display: flex; justify-content: space-between; margin: 15px 0 0 0; padding-top: 15px; border-top: 2px dashed #fbbf24;">
								<span style="color: #f59e0b; font-size: 20px; font-weight: bold;">Total:</span>
								<span style="color: #d97706; font-size: 24px; font-weight: bold;">${originalCard.querySelector('.text-sky-700 span:last-child').textContent}</span>
							</div>
						</div>
					</div>
					...
					<div style="background: white; padding: 18px; border-radius: 10px; box-shadow: 0 8px 20px rgba(0,0,0,0.15);">
						<h3 style="color: #f59e0b; font-size: 18px; margin: 0 0 12px 0; border-bottom: 2px solid #fbbf24; padding-bottom: 8px;">Delivery & Payment Details</h3>
						<div style="margin-bottom: 15px;">
							<p style="color: #f59e0b; font-weight: bold; margin: 0 0 8px 0; font-size: 14px;">Delivery Address:</p>
						...
						<div style="margin-bottom: 15px;">
							<p style="color: #f59e0b; font-weight: bold; margin: 0 0 8px 0; font-size: 14px;">Payment Method:</p>
						...
						${originalCard.querySelector('.space-y-3 > div:nth-child(3)') ? `
							<div>
								<p style="color: #f59e0b; font-weight: bold; margin: 0 0 8px 0; font-size: 14px;">Special Instructions:</p>
					...
					<div style="text-align: center; margin-top: 15px; padding: 15px; background: rgba(255,255,255,0.15); border-radius: 10px;">
						<p style="color: white; margin: 0; font-size: 13px; text-shadow: 1px 1px 2px rgba(0,0,0,0.3);">Thank you for choosing Campus Cravings - Neptune Kitchen!</p>
