...
$sqlRevenue = "SELECT DATE(order_date) as od, SUM(total_amount) as rev
               FROM orders
               WHERE LOWER(kitchen) = 'neptune' AND order_status = 'delivered'
...
$sqlStatus = "SELECT order_status, COUNT(*) AS c FROM orders WHERE LOWER(kitchen) = 'neptune' GROUP BY order_status";
...
// Recent reviews
$sqlRecent = "SELECT pr.*, n.name as product_name, c.fullname, 
              DATE_FORMAT(pr.created_at, '%b %d, %Y') as review_date
              FROM product_reviews pr
              JOIN neptune n ON pr.product_code = n.product_code
              JOIN customer c ON pr.customer_id = c.customer_id
              WHERE pr.kitchen = 'neptune'
...
<HEAD>
    ...
    <title>Neptune Kitchen - Statistics</title>
...
<body class="bg-gray-50 text-gray-900">
    ...
    <section id="hero" class="hero">
        ...
        <div class="relative z-10 max-w-4xl mx-auto px-4 py-20 text-center text-white">
            ...
            <p class="text-lg text-gray-200">Sales performance for Neptune Kitchen</p>
        </div>
    </section>
